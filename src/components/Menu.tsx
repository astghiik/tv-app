...
function Menu() {
  return (
    <aside className="p-9 fixed z-50 group flex flex-col justify-between gap-8 left-0 h-full overflow-x-hidden overflow-y-auto hide-scrollbar">
      <div>
        <div className="opacity-0 transition-opacity duration-300 group-hover:opacity-100 flex text-2xl font-semibold items-center gap-6 mb-5">
        ...
        <ul className="flex flex-col gap-4 transition-all duration-300 w-[78px] group-hover:w-[270px]">
      ...
      <div className="opacity-0 transition-opacity duration-300 group-hover:opacity-100 flex items-start gap-3 flex-col uppercase text-xl text-gray font-semibold px-6">
        <a href="#">language</a>
        <a href="#">get help</a>
        <a href="#">exit</a>
      </div>
      <div className="hidden fixed -z-10 left-0 top-0 right-0 bottom-0 group-hover:block bg-[linear-gradient(90deg,#040404_0%,#040404FA_21%,#04040400_100%)]"></div>
    </aside>
  );
}
